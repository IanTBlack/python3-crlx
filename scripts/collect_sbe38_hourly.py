...
def main():
    ...
    crlx = SIKULIAQ()
    ds = crlx.get_sbe38(bdt, edt)
    ds2 = crlx.get_sbe45(bdt,edt)
    ...
    for sensor_id in sensor_ids: #In the event of multiple sensors being recorded over the request timespan, split the saved data into multiple files.
        ...
        _ds['qartod_climatology_sea_surface_temperature'] = climatology_test(_ds.sea_surface_temperature,_ds.latitude,_ds.longitude,lookup)
        _ds['qartod_neighbor_sea_surface_temperature'] = neighbor_test(_ds.sea_surface_temperature, ds2.sea_water_temperature)
        _ds['qartod_multivariate_sea_surface_temperature'] = multi_variate_test(_ds.sea_surface_temperature,ds2.sea_water_practical_salinity)
