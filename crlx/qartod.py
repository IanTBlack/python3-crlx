...
def location_test(latitude: xr.DataArray, longitude: xr.DataArray) -> xr.DataArray:
    ...
    :param latitude: xr.DataArray of latitude values with a 'time' coordinate.
    :param longitude: xr.DataArray of longitude values with a 'time' coordinate that has the same timestamps as the latitude.
    ...
    results = results.where(~np.isnan(latitude) | ~np.isnan(longitude), FLAG.MISSING_DATA)  # If the inputs are NaN, mark as missing.

    results.attrs['test_name'] = 'QARTOD Location Test'
    results.attrs['is_qartod'] = "True"
    results.attrs['is_experimental'] = "False"
    results.attrs[
        'description'] = "The QARTOD Location Test verifies if a given latitude and longitude for an associated data point are within the confines of reality. Reality is defined as -90 to 90 for latitude and -180 to 180 for longitude. Implementation of this test does not currently support the flagging of suspect values."
    results.attrs['valid_flags'] = "1 = PASS, 2 = NOT_EVALUATED, 4 = FAIL, 9 = MISSING_DATA"
    results.attrs['valid_latitude_min'] = -90
    results.attrs['valid_latitude_max'] = 90
    results.attrs['valid_longitude_min'] = -180
    results.attrs['valid_longitude_max'] = 180
    return results
...
def gross_range_test(data: xr.DataArray, sensor_min: float, sensor_max: float,
    ...
    results.attrs['is_experimental'] = "False"
    results.attrs[
        'description'] = "The QARTOD Gross Range Test verifies if a given data point is within the limits defined by the sensor manufacturer (and operator)."
    results.attrs[
...
def climatology_test(data: xr.DataArray,
    ...
    results.attrs['is_experimental'] = "False"
    results.attrs[
        'description'] = "The QARTOD Climatology Test verifies if a sample is within the minimum and maximum expected value for a given time of the year and location. The nearest grid cell and time from the reference dataset are used when flagging data."
...
def spike_test(data: xr.DataArray, std_window: int = 1 * 30 + 1, low_multiplier=3, high_multiplier=5):
    ...
    results.attrs['is_experimental'] = "False"
    results.attrs[
        'description'] = "The QARTOD Spike Test uses the neighboring points (n-1, n+1) to determine if data (n) is unusually high or low, indicative of a spike that could be representative of unusual environmental variation or sensor failure depending on the sampling rate."
    results.attrs[
        'valid_flags'] = "1 = PASS, 2 = NOT_EVALUATED, 3 = HIGH_INTEREST or SUSPECT, 4 = FAIL, 9 = MISSING_DATA"
...
def rate_of_change_test(data: xr.DataArray, std_multiplier: int = 3):
    ...
    results.attrs['is_experimental'] = "False"
    results.attrs[
        'description'] = "The QARTOD Rate of Change Test assesses the rate of change of a variable over time. Currently the implementation of this test estimates the rate of change between each data point, regardless of the sampling rate. This test does not flag BAD or FAIL data."
    results.attrs['valid_flags'] = "1 = PASS, 2 = NOT_EVALUATED, 3 = HIGH_INTEREST or SUSPECT, 9 = MISSING_DATA"
...
def flat_line_test(data: xr.DataArray, fail_window_size: int, suspect_window_size: int, eps: float) -> xr.DataArray:
    ...
    results.attrs['is_experimental'] = "False"
    results.attrs[
        'description'] = "The QARTOD Flat Line Test assesses if data are reasonably variable. Failure of the flat line may suggest that the sensor is repeating values or is obstructed. "
    results.attrs[
        'valid_flags'] = "1 = PASS, 2 = NOT_EVALUATED, 3 = HIGH_INTEREST or SUSPECT, 4 = FAIL, 9 = MISSING_DATA"
...
def multi_variate_test(data1: xr.DataArray, data2: xr.DataArray,
    ...
    if sel_method is not None:  # If the user does not provide a selection method, it is assumed that data2 has the same timestamps as data1.
    ...
    results.attrs['is_experimental'] = "False"
    results.attrs[
        'description'] = "The QARTOD Multi-Variate Test assesses if variable #1 varies at similar rate of change compared to variable #2. In this implementation of the test, the rate of change test is run on each variable. If variable #2 passes the RoC test, but variable #1 does not, data are flagged as suspect. This test does not fail data."
    results.attrs['valid_flags'] = "1 = PASS, 2 = NOT_EVALUATED, 3 = HIGH_INTEREST or SUSPECT, 9 = MISSING_DATA"
...
def neighbor_test(data1: xr.DataArray, data2: xr.DataArray,
    ...
    results.attrs['is_experimental'] = "False"
    results.attrs[
        'description'] = "The QARTOD Neighbor Test assesses if variable #1 varies at similar rate of change compared to variable #2. It is similar to the multi-variate test, but the expectation is that both input variables represent the same product from two different sensors. In this implementation of the test, the rate of change test is run on each variable. If variable #2 passes the RoC test, but variable #1 does not, data are flagged as suspect. This test does not fail data."
    results.attrs['valid_flags'] = "1 = PASS, 2 = NOT_EVALUATED, 3 = HIGH_INTEREST or SUSPECT, 9 = MISSING_DATA"
    results.attrs['variable_1'] = data1.name
    results.attrs['variable_2'] = data2.name
    results.attrs['variable_1_multiplier'] = data1_multiplier
